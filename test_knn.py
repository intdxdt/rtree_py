...
def fn_rich_data():
	richData = []
	data = [
		[1, 2, 1, 2], [3, 3, 3, 3], [5, 5, 5, 5], [4, 2, 4, 2], [2, 4, 2, 4], [5, 3, 5, 3],
		[3, 4, 3, 4], [2.5, 4, 2.5, 4],
	]
...
def scoreFn(query, boxer):
	q = MBR(query[0], query[1], query[0], query[1])
	if isinstance(boxer, Polygon):
		o = MBR(*boxer.bounds)
	else:
		o = MBR(*boxer[:4])
	return q.distance(o)
...
class TestRTree_KNN(unittest.TestCase):
	def test_knn(self):
		...
		data = init_knn_data()
		rt.load(data)
		nn = rt.knn((40, 40), score_func=scoreFn, limit=10)
		...
		nn = rt.knn((40, 40), score_func=scoreFn, limit=1000)
	...
	def test_rtree_knn_predicate(self):
		...
		def scoreFunc(query, obj):
			if isinstance(obj, MBR):
				...
				dist = query.distance(MBR(*obj.bounds))
			else:
		...
		res = rt.knn(query, limit=10, score_func=scoreFunc, predicate=prefFn)
	...
	def test_knn_predicate(self):
		...
		query = (2, 4)
		result = rt.knn(query, limit=1, predicate=pred, score_func=scoreFn)
